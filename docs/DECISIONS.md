...
## DEC-023 — 2026-09-23 — Entrega local robusta sin promesa de puntualidad
El usuario eligió "entrega local robusta" frente a puntualidad agresiva local o backend+FCM. El outbox durable se reintenta al inicio de cada corrida (aunque luego falle la red), al abrir la app (ON_RESUME, sin red) y su detalle es visible en el diagnóstico. Las alertas con etapa confirmada esperan una revisión fresca con red y nunca se entregan desde fuentes no rechecadas. La decisión por evento es pura y testeada (DELIVER/KEEP/DROP). No se promete puntualidad de 15 min (Doze/OEM mandan) y no se revierte DEC-006/008.

## DEC-022 — 2026-09-21 — Trabajo activo, refresco por ciclo de vida y reanudación con UPDATE
Correcciones de la auditoría QA de Devin sobre ac8e6b4. (1) La observación del trabajo periódico selecciona el trabajo activo (ENQUEUED o RUNNING) de forma determinística; `getWorkInfosForUniqueWorkFlow` puede devolver entradas históricas terminadas (CANCELLED/SUCCEEDED/FAILED) junto con el trabajo actual, y un histórico terminado nunca debe producir un falso AT_RISK si existe un trabajo activo; sin activo se devuelve ausencia de programación (null) que el diagnóstico interpreta como AT_RISK salvo pausa. (2) El entorno Android (permiso de notificaciones, canales y optimización de batería) se re-lee solo ante triggers de ciclo de vida — `LifecycleEventEffect(ON_RESUME)` al regresar a la app y `LaunchedEffect(tab)` al entrar a Ajustes — y nunca con polling continuo. (3) El arranque normal conserva `ExistingPeriodicWorkPolicy.KEEP` (no recrea un trabajo ya programado); un cambio de frecuencia usa UPDATE y la reanudación tras pausar usa el nuevo `CnscMonitoringWorker.resume` también con UPDATE sobre el intervalo persistido (actualiza el pendiente o re-encola tras una cancelación, dejando exactamente un trabajo periódico activo, sin tocar following ni Room). No se usa REPLACE por estar deprecado. Motivo: decisiones informadas sin falsas garantías y sin falso diagnóstico por trabajos históricos; comportamiento robusto de pausa/reanudación sin ambigüedad.

## DEC-021 — 2026-09-21 — Diagnóstico honesto y preferencias cumplibles
El diagnóstico (pestaña Ajustes) se construye exclusivamente de fuentes reales (Room: notice_state y activity_check; WorkManager: estado y próxima ejecución del trabajo único; configuración Android: permiso, canales y optimización de batería). Un dato no disponible se muestra como desconocido o "sin dato", nunca se inventa (DEC-011 "nunca presentar inferencias como oficiales"). El estado general tiene precedencia: pausada > sin trabajo/estado no activo > sin primera revisión > degradada (fallo reciente de micrositio en 24 h o permiso de notificaciones denegado) > operativa. Las preferencias solo exponen lo que la arquitectura puede cumplir: frecuencia de revisión (15/30/60/120 min, mínimo real de WorkManager; se reprograma el trabajo único con ExistingPeriodicWorkPolicy.UPDATE y persiste en monitor_schedule) y pausar/reanudar vigilancia (cancela/reprograma el trabajo único con monitoring_paused_v1). Ajustes NO modifica following (solo enlaza a Concursos) ni promete ejecución exacta a los 15 minutos: WorkManager lo solicita y Android puede retrasarlo o agruparlo. No hay toggle global de notificaciones: se delega en Android (DEC-008/DEC-015) y el diagnóstico refleja el estado real. Motivo: decisiones informadas sin falsas garantías y sin opciones que la arquitectura local-first no pueda honrar; sin backend y sin requisito de instalación nueva.

## DEC-020 — 2026-09-21 — Historial robusto: compromiso total o nada
La lectura paginada del micrositio se compromete solo con un recorrido completo exitoso: un fallo en una página posterior se propaga (no se convierte en éxito parcial) y la cancelación se propaga igualmente; la caché previa se conserva intacta porque nada se persiste a medias. La consulta manual descarga la primera página una sola vez compartiéndola entre identidad y actividad (`ProcessMicrositeReader`), y una pausa de 2 s separa las solicitudes sucesivas a CNSC en este recorrido. Cada publicación conserva la URL real de la página donde se obtuvo. Motivo: un historial parcial no debe sustituir a uno más completo ya guardado, y toda URL de evidencia debe corresponder a la página que efectivamente la contenía. Implica regresión sobre la política anterior (DEC-018 toleraba y conservaba lo recogido ante fallos tardíos): ahora es "commit solo con recorrido completo exitoso".

## DEC-019 — 2026-09-21 — Migraciones Room instrumentadas como verificación real
Los tests de migración instrumentados no son opcionales: se corrigen y se ejecutan en dispositivo con datos propios. La suite usa una DB de prueba aislada ("migration-test") y los esquemas históricos exportados (schemas/*.json 1/2/3) como fuente de verdad; no desinstala ni borra radar.db ni los datos reales. La verificación en el primer pase detectó un error real de esquema (MIGRATION_2_3 añadía slug nullable mientras la entidad la declara NOT NULL), corregido a "ALTER TABLE processes ADD COLUMN slug TEXT NOT NULL DEFAULT ''". Motivo: una migración que produce un esquema distinto del declarado es un bug silencioso a futuro; la suite la detecta comparando TableInfo contra el JSON destino. Nota operativa: el runner connectedDebugAndroidTest desinstala la app antes de instalar el APK de prueba; la ejecución en dispositivo se hace con pm install -r + am instrument para conservar datos del teléfono.
